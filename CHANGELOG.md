...
Real-Postgres integration testing caught a genuine production bug along the way: the claim-and-run lease `UPDATE` sent a Python `datetime` as `text`, rejected by the `timestamp without time zone` column — no unit test could have caught this, since every unit test mocks the DB. Fixed with an explicit `timestamptz` cast; see ADR-0033's completion addendum for this and other deviations/fast-follows.

Full TDD across all 16 tasks, 35 commits, 1099 tests passing. See ADR-0033 in `docs/decisions.md` for the complete design record (options considered, the cost-driven sweeper redesign, and the completion summary) and `docs/architecture.md`'s new "Durable execution, events, and rate limits" section for the resulting flow.
