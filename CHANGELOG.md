...
- `0cf784c` — vectordb-provider batch: all 7 files (Chroma/Qdrant/Pinecone/Weaviate/Milvus + `embedding.py` + `providers/base.py`), 130 → 0 warnings, entirely via `cast()`/annotations at the SDK-response boundary — no ignores needed.
- `7fb853a` — final batch: the remaining 37 production files (API routes, engine, executors, MCP client/OAuth, tools, variable substitution/validation), 266 → 0 warnings. Same `cast()`-after-`isinstance()` treatment applied identically to all four secret-redaction helpers in `src/api/workflows.py` and to the OAuth token/scope handling in `src/mcp/oauth.py` — both security-sensitive, both diff-reviewed to confirm zero logic change, annotations only. Remaining warnings that were genuine third-party stub gaps (not our code) fell back to narrow `# pyright: ignore[...]`: `langgraph.graph`, `xhtml2pdf`, `google-auth`, `google-cloud-tasks`, and several LangChain `bind_tools`/`ainvoke`/`.content` call sites.
...
**Hard isolation boundary, by design**: TypeSafe is reachable only from the `decision` node, never from any other node's `model` field or the shared `build_chat_model` dispatcher — enforced by omission (no `"typesafe"` branch in `src/llm/providers.py`, no `"typesafe"` entry in any other panel's `PROVIDER_OPTIONS`) and locked by a permanent regression test (`tests/unit/llm/test_providers_isolation.py`). TypeSafe otherwise gets full admin-catalog parity with the four existing LLM providers — key CRUD, test-connection probe, per-model verify probe, and both admin frontend pages — since none of that grants execution-path reachability by itself.

`DecisionNode`/`DecisionNodeData` (`src/engine/workflow.py`), `DecisionExecutor` (`src/executors/decision.py`), routing via `_route_decision` in `src/engine/graph_builder.py` (choice mode required generalizing the existing fixed `{true,false}`/`{body,exit}`/`{approved,rejected}` branch-key machinery to a dynamic per-node set), full Designer UI (palette entry, dynamic per-option branching handles on the canvas, a configuration panel with mode/provider toggles, an examples/options list editor), and Template 21 (`scripts/seed_templates.py`) as the reference workflow.

Built via a 15-task TDD plan (`docs/superpowers/plans/2026-09-22-decision-node.md`, spec at `docs/superpowers/specs/2026-09-22-decision-node-design.md`), each task independently reviewed; final whole-branch review clean, zero Critical/Important findings. Backend +42 tests (1256 → 1298), frontend +9 (164 → 173), full e2e smoke test (`frontend/e2e/decision-node.spec.ts`) run twice green against live servers — not a mocked skeleton. No DB migration anywhere in the feature (`Workflow.nodes`/`edges` are `Json` columns; provider columns are free strings, not enums).

One real, pre-existing bug surfaced along the way, confirmed out of scope for this feature and not fixed here: React Flow's click-to-connect reliably fails when the source is a *labeled* branch handle, reproduced on both the new `decision` node and the existing `if-else` node — a general `BranchingNode` interaction issue, not something this feature introduced. The e2e test works around it with a direct mouse drag (`locator.dragTo()`) rather than click-to-connect.

### Fixed — TypeSafe judgment calls 422'd with no explicit model set (2026-09-23)

User-reported, caught testing the just-shipped feature above: clicking "Test connection" on a saved TypeSafe key returned `HTTP 422: {"detail":[{"type":"missing","loc":["body","model"] ...}]}`. Root cause: TypeSafe's live `/v1/systemone` rejects a request with no `model` field outright — the docs' claim that it defaults to `jev-latest` server-side turned out to be a client-side-only default in TypeSafe's own Python SDK, not something the raw HTTP API does. `TypeSafeJudgmentProvider.decide_binary`/`decide_choice` (`src/llm/judgment.py`) only sent `model` when a `decision` node explicitly configured one, so **every** node using the TypeSafe provider without an explicit model was failing 100% of the time in production, not just the admin test probe that happened to surface it first.

Both methods now always send `model or "jev-latest"` (`_DEFAULT_TYPESAFE_MODEL`); same fix applied to the admin key-test probe (`src/api/admin_llm_keys_test.py`), which had the identical gap. `_verify_typesafe` was already unaffected — it always operates on a specific `model_id`. Regression tests added reproducing the exact failure (missing-field body → `model` defaults correctly).

### Fixed — Admin Add-Model empty-state message misleading for TypeSafe (2026-09-23)

User-reported, same testing session: selecting "TypeSafe (Jev)" in the Add-Model dialog with no rows yet configured showed *"No new typesafe models to add — everything the provider exposes is already in the table"* — accurate for a provider with a live catalog (Anthropic, OpenAI, etc.) fully covered by existing rows, actively misleading for TypeSafe, which has no live discovery at all and had nothing in the table yet. `frontend/app/admin/llm-models/page.tsx` gains a frontend-only `DB_ONLY_PROVIDERS` mirror of the backend's `_DB_ONLY_PROVIDERS` (`src/api/llm_models_live.py`) purely to word this one message correctly — the backend already enforced the real DB-only-fallback behavior regardless.

### Fixed — CI's full-project pyright scan caught 7 errors a scoped check missed (2026-09-23)

`tests/unit/engine/test_workflow_decision.py` (Task 1 of the Decision node plan, above) passed every pyright check run *during* that plan's task-by-task review — but those were all narrow, 1-2-file invocations (e.g. `pyright src/engine/workflow.py tests/unit/engine/test_workflow_decision.py`). The push that shipped Template 21 (above) triggered CI's actual check, `pyright src tests` over the whole project, which failed with 7 real errors in that same file: two untyped `**data_overrides` kwargs and three unguarded reads of `Optional` fields (`data.options`, `data.examples`) with no narrowing `assert`. Fixed directly on `main`. Process note for future verification: a scoped pyright pair is not a substitute for the exact command CI runs — confirmed separately that the GCP Cloud Run deploy workflow does **not** gate on CI passing (it ran and succeeded on the broken commit independently), so a red CI silently doesn't block production either; worth revisiting as a CI/CD hygiene gap, not fixed here.
...
Start-node input fields (e.g. `report_date`, `extract_timestamp` in a customer's Jira weekly delivery tracking workflow) previously only had `text`/`number`/`boolean`/`json`/`document` types, forcing end-users to type dates by hand. Adds `date` and `datetime` as two new field types, each rendering a calendar picker instead of a plain text box, everywhere a Start field's value is entered.
