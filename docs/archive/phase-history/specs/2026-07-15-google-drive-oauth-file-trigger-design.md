...
- `GET /cloud-storage/google-drive/authorize` — builds the Google consent URL (`https://accounts.google.com/o/oauth2/v2/auth`) with `scope=https://www.googleapis.com/auth/drive.file`, `access_type=offline` (required to receive a refresh token), `prompt=consent`, redirects the browser.
