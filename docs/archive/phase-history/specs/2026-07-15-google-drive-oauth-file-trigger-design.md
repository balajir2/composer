...
- `GET /cloud-storage/google-drive/authorize` — builds the Google consent URL (`https://accounts.google.com/o/oauth2/v2/auth`) with `scope=https://www.googleapis.com/auth/drive.file https://www.googleapis.com/auth/userinfo.email`, `access_type=offline` (required to receive a refresh token), `prompt=consent`, redirects the browser. The `userinfo.email` scope was added 2026-07-18 after production testing found the callback's userinfo lookup (below) fails with a 401 on `drive.file` alone — that scope grants no access to the userinfo endpoint, even though the token exchange itself succeeds.
...
**OAuth scope note:** `drive.file` only grants access to files the app *created* or that the user explicitly *selected*. For an ongoing folder watch (seeing files added *after* the grant), the folder itself must be selected through the **Google Picker API** in folder-select mode — Google documents this as granting ongoing access to the folder's current and future contents. This is why §E's "Connect" UX is not just an OAuth redirect. (The alternative — requesting the broader `drive` scope to avoid Picker — was explicitly rejected: `drive` is a Google-restricted scope requiring a costly annual security assessment for verified production use, a bad trade for an internal tool.)
