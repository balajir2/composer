...
**Login enforcement:** `POST /auth/login` (`auth_standalone.py`), on success, checks `mustChangePassword`. If true, it does **not** issue a normal access/refresh pair; instead it issues a `password_change` token — a third JWT type alongside the existing `access`/`refresh` discriminator in `src/security/jwt.py` (`AccessTokenPayload.type`, `RefreshTokenPayload.type`), with a short TTL (10 minutes) — and returns `{"must_change_password": true, "password_change_token": "..."}` instead of the normal token pair. Because every existing route dependency (`get_current_user_id` in `src/security/auth.py`) already rejects any JWT whose `type` isn't `"access"`, a `password_change` token is automatically unusable everywhere except the one new endpoint that explicitly verifies it — no changes needed to existing route dependencies. The frontend routes the user to a forced change-password screen using that token. After a successful `POST /auth/change-password`, `mustChangePassword` clears and the user must log in again to get a normal session.
