...
**Status.** Accepted 2026-07-13, implemented 2026-07-15. User approved: Cloud Tasks as new infrastructure, full replacement of the in-process execution model (not run-alongside), and P1-2 + P1-4 implemented together. Implementation plan: `docs/superpowers/plans/2026-07-13-durable-execution-cloud-tasks.md` (16 tasks, TDD) — all 16 complete, 35 commits. See the 2026-07-14 addendum below for a cost-driven refinement to Task 11/14's scope, made mid-implementation, and the 2026-07-15 addendum for a completion summary (deviations, the production bug Task 15's integration tests caught, and the main/branch divergence on `scripts/gcp-bootstrap.ps1` this leaves for merge time).
...
**Addendum (2026-07-15) — completion summary.** All 16 tasks landed (35 commits; see Implemented by below). Notable deviations/discoveries beyond the Task 11 sweeper redesign above:
...
- **This plan required substantially more review/fix cycles than a typical Composer feature of similar size**, proportionate to what `claim-and-run` actually is: the one endpoint responsible for guaranteeing an execution runs exactly once despite Cloud Tasks' at-least-once delivery — a security- and data-loss-sensitive surface where a subtle bug (a missing status filter, a race in the fast-fail write, an unguarded concurrent cancel) means either a silently double-run workflow (duplicate Jira tickets, duplicate emails) or a silently lost one. Most tasks in the 9-13/15 range shipped a `feat` commit followed by one or more same-review-pass `fix` commits rather than landing clean on the first pass (see Implemented by below) — treated as the expected cost of that sensitivity, not a process failure.

**Known unresolved item at completion — main/branch divergence.** This branch forked from `main` before `main`'s `db66110` ("fix(ops): drop --no-cpu-throttling, redesign sweeper scope") landed; this branch independently made the same `--no-cpu-throttling` removal (see "Interim action taken" above) on its own fork of `scripts/gcp-bootstrap.ps1`, and this ADR's own text now exists in two textually-independent forms — one on `main` (from `db66110`), one on this branch. Both describe the same outcome, but reconciling the two file versions is a merge-time concern for whoever integrates this branch into `main`; not attempted or resolved as part of Task 16.
...
- Task 14 (`9ea55a2`, `cb5ddd6`) — Cloud Tasks queue + OIDC service-account provisioning (`scripts/gcp-bootstrap.ps1`); runtime service-account enqueue-permission fix. Diverges from `main`'s independent `db66110` fix to the same file — see "Known unresolved item" above.
...
- Task 16 (this commit) — ADR status/completion summary, `docs/architecture.md`, `docs/deferred-backlog.md`, `CHANGELOG.md`.

All commits on branch `feat/durable-execution-cloud-tasks` (forked from `main` at `9f1b131`), worktree `D:/GitHub/composer.worktrees/feat-durable-execution`.

**Related.** `docs/claude-improvement-backlog.md` §P1-2 and §P1-4; `docs/deferred-backlog.md` (P1-2/P1-4 entries); ADR-0031 (the row-locking POC this reuses); `scripts/poc_persistence_row_lock.py`; `src/maintenance/execution_sweeper.py` (the existing sweep pattern this extends or replaces); `scripts/gcp-bootstrap.ps1` (the deployment config this analysis is grounded in, and the file this branch diverges from `main` on — see "Known unresolved item" above); `docs/superpowers/plans/2026-07-13-durable-execution-cloud-tasks.md` (implementation plan, Task 11/14 updated per the 2026-07-14 addendum, all 16 tasks complete per the 2026-07-15 addendum).
