...
4. **LLM keys in Postgres; Vercel sync at deploy time.** The deploying organization owns the source of truth. Runtime reads env vars (unchanged); `composer keys sync --target vercel` pushes decrypted values to Vercel env via Vercel API. Same AES-256-GCM encryption mechanism as MCP OAuth tokens.
...
4. **Tailwind + shadcn/ui over Ant/Mantine.** Installed `base-nova` style (on `@base-ui/react`, not Radix — the default shipped by `shadcn init` as of v4); CSS variables hook for future re-theming. `Button asChild` is unsupported in this style; use `<Link>` + `buttonVariants()`.
